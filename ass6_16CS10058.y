...
primary_expression
	...
	ProgramStrings.push_back($1);
	stringstream strs;
    strs << ProgramStrings.size()-1;
...
constant
	...
	|FLOATING_CONSTANT {
	$$ = gentemp(new symboltype("float"), string($1));
	emit("=", $$->name, string($1));
	}
...
multiplicative_expression
	...
	|multiplicative_expression MUL cast_expression {
		if (checksymboltype ($1->loc, $3->array) ) {
			$$ = new expr();
